...
def html2pdf(html, out_path):
    ...
    """
    if not html:
        log(f"Cannot generate PDF: HTML content is empty")
        return False
...
def upload_to_supernote(pdf_files):
    ...
    if not dir_id:
        ...
        log("Please set SUPERNOTE_DIR_ID after first run by finding the HackerNews folder ID")
    ...
    try:
        ...
        if not dir_id:
            ...
            for item in root_files:
                if item['isFolder'] == 'Y' and item['fileName'] == "HackerNews":
                    dir_id = item['id']
            ...
            if not dir_id:
                log("ERROR: HackerNews folder not found. Please create it manually in Supernote cloud")
                return 0
    ...
    try:
        ...
        if pdf_files:
            try:
                ...
                log(f"Uploaded {uploaded_count} of {len(pdf_files)} files to Supernote cloud")
            except Exception as e:
